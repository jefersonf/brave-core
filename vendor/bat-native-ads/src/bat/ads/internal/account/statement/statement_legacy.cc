...
// TODO(tmancey): Dependency inject transactions and return statement for date
// range
StatementInfo Statement::Get(const base::Time& from,
  ...
  statement.ads_received_this_month = GetAdsReceivedForThisMonth();

  statement.earnings_this_month = GetEarningsForThisMonth();
  statement.earnings_last_month = GetEarningsForLastMonth();

  statement.transactions = transactions::GetCleared(from, to);
...
uint64_t Statement::GetAdsReceivedForThisMonth() const {
  const base::Time now = base::Time::Now();
